...
def delete_file(filename):
    try:
        ...
        st.warning(f"⚠️ File '{filename}' not found.")
    except Exception as e:
        st.error('❌ An error occurred!')
...
def read_file(filename):
    try:
        ...
        st.warning(f"⚠️ File '{filename}' doesn't exist.")
    except Exception as e:
        st.error('❌ An error occurred!')
...
def edit_file(filename, content):
    try:
        with open(filename, 'a') as f:
            f.write(content + "\n")
            st.success(f"✏️ File '{filename}' edited successfully.")
    except FileNotFoundError:
        st.warning(f"⚠️ File '{filename}' doesn't exist.")
    except Exception as e:
        st.error('❌ An error occurred!')
...
def main():
    ...
    option = st.selectbox('Select an action:', ['📄 Create File', '👁️ View All Files', '🗑️ Delete File', '📖 Read File', '✏️ Edit File', '🚪 Exit'])
    ...
    elif option == '👁️ View All Files':
        if st.button('View Files 👁️‍🗨️'):
            view_all_files()
